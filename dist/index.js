...
// src/core/context/turn-context-collector.ts
function clampWindowSize(value) {
  return Number.isFinite(value) && value > 0 ? Math.floor(value) : 1;
}
var TurnContextCollector = class {
  collect(messages, triggerRawIndex, analysisMode, windowSize) {
    const turns = [];
...
function getMessageIndexFromPayload(payload, context) {
  ...
  if (payload && typeof payload === "object") {
    ...
    const directMessage = payloadObject.message && typeof payloadObject.message === "object" ? payloadObject.message : payloadObject.mes && typeof payloadObject.mes === "object" ? payloadObject.mes : "is_user" in payloadObject || "mes" in payloadObject || "name" in payloadObject ? payloadObject : null;
    if (directMessage) {
      const chat = getChatMessages(context);
...
function getAnalysisMessages(context) {
  const chat = getChatMessages(context);
  ...
  for (let index = 0; index < chat.length; index += 1) {
    ...
    const role = isUserMessage(message) ? "user" : isCharacterMessage(message) ? "character" : null;
    if (!role) {
      continue;
    }
